...
#define JsBridgeCoreFileName @"GTJSBridge.js.txt"  //核心JS文件默认为这个名字
@implementation GTJSExportDetail
@end


@implementation GTJSPluginInfo
...
@implementation GTJSPluginManager
...
- (void)resetWithConfigFile:(NSString *)file
{
    ...
    if (path) {
        ...
        for (NSDictionary *plugin in plugins) {
            GTJSPluginInfo *info = [[GTJSPluginInfo alloc] init];
            ...
            for (NSDictionary *exportInfo in [plugin objectForKey:@"exports"]) {
                GTJSExportDetail *tmp = [[GTJSExportDetail alloc] init];
...
{
    return ((GTJSPluginInfo *)[self.pluginMap objectForKey:pluginName]).instance;
}
...
- (NSString *)realForShowMethod:(NSString *)showMethod
{
    ...
    for (NSString *key in _pluginMap.allKeys) {
        GTJSPluginInfo *tmp = [_pluginMap objectForKey:key];
