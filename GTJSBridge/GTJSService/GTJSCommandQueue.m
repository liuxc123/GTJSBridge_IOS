...
@interface GTJSCommandQueue () {
    __weak GTJSService *_jsService;
...
@implementation GTJSCommandQueue
        ...
        while ([_queue count] > 0) {
            ...
            while ([commandBatch count] > 0) {
                @autoreleasepool
                {
                    ...
                    GTJSInvokedUrlCommand *command =
                    [GTJSInvokedUrlCommand commandFromJson:jsonEntry];
...
- (BOOL)execute:(GTJSInvokedUrlCommand *)command
{
    ...
    GTJSPlugin *obj = [_jsService getPluginInstance:command.pluginName];
    if (!obj || !([obj isKindOfClass:[GTJSPlugin class]])) {
        NSLog(@"ERROR: Plugin '%@' not found, or is not a GTJSPlugin. Check your plugin mapping in "
              @"PluginConfig.json.",
              command.pluginName);
        return NO;
    }
