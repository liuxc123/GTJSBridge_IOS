...
- (void)getDeviceInfo:(GTJSInvokedUrlCommand *)command
{
    ...
    GTJSPluginResult *pluginResult = [GTJSPluginResult
                                      resultWithStatus:GTJSCommandStatus_OK
...
- (void)getClientInfo:(GTJSInvokedUrlCommand *)command
{
    ...
    GTJSPluginResult *pluginResult =
    [GTJSPluginResult resultWithStatus:GTJSCommandStatus_OK messageAsDictionary:clientInfos];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
...
- (void)getNetworkInfo:(GTJSInvokedUrlCommand *)command
{
    ...
    GTJSPluginResult *pluginResult =
    [GTJSPluginResult resultWithStatus:GTJSCommandStatus_OK messageAsInt:netType];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
...
{
    GTJSPluginResult *pluginResult =
    [GTJSPluginResult resultWithStatus:GTJSCommandStatus_OK messageAsInt:1];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
...
- (void)setScreenStatus:(GTJSInvokedUrlCommand *)command
{
    ...
    [dic_result setObject:(curStatus == 0 ? @"当前屏幕处于不长亮状态"
                           : @"当前屏幕处于常亮状态")
                   forKey:@"message"];
    
    GTJSPluginResult *pluginResult =
    [GTJSPluginResult resultWithStatus:GTJSCommandStatus_OK
